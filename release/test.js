...
require(['QUnit', 'jsiq'], function(QUnit, jsiq)
{
	...
	{		
		...
		check(jsiq.query('for $x in (1, 2, 3) for $y in ( 1, 2, 3 ) return 10 * $x + $y'), [11, 12, 13, 21, 22, 23, 31, 32, 33], 'for for flowr');
		
		check(jsiq.query('for $x in (1, 2, 3), $y in ( 1, 2, 3 ) return 10 * $x + $y'), [11, 12, 13, 21, 22, 23, 31, 32, 33], 'double for flowr');
		
		check(jsiq.query('for $x in ( [ 1, 2, 3 ], [ 4, 5, 6 ], [ 7, 8, 9 ] ), $y in $x[] return $y'), [1, 2, 3, 4, 5, 6, 7, 8, 9], 'dependent for flowr');
		
		check(jsiq.query('for $x in ({"name":["john", "doe"]}, {"name":["bob", "mcgee"]}), $y in $x.name[] return $y'), ["john", "doe", "bob", "mcgee"], 'dependent for object lookup flowr');
		
		check(jsiq.query('for $x at $i in ("one", "two") return {"i": $i, "name": $x}'), [{i:1, name: "one"}, {i:2, name:"two"}], 'for position flowr');
		
		check(jsiq.query('for $x in ("one", "two"), $y allowing empty in () return {"x": $x, "y": $y.name}'), [{x:"one", y:null}, {x:"two", y:null}], 'for empty flowr');
		
		check(jsiq.query('for $x in (1, 2, 3, 4) where $x mod 2 eq 0 return $x'), [2, 4], 'where flowr');
		
		check(jsiq.query('for $x in (1, 3, 2, null) order by $x return $x'), [null, 1, 2, 3], 'order by flowr');
		
		check(jsiq.query('for $x in (1, 3, 2, null) order by $x descending empty least return $x'), [3, 2, 1, null], 'order by flowr');
		
		check(jsiq.query('for $x in (1, 3, 2, null) order by $x ascending empty greatest return $x'), [1, 2, 3, null], 'order by flowr');
		
		check(jsiq.query('for $x in ({order: 2, a: []}, {order: 1, a: [1, 2]}, {order: 1, a: []}) order by $x.order, $x.a.length return $x'),
			[{order: 1, a: []}, {order: 1, a: [1, 2]}, {order: 2, a: []}], 'double order by object flowr');
			
		check(jsiq.query('for $x in ({order: 2, a: []}, {order: 1, a: [1, 2]}, {order: 1, a: []}) order by $x.order descending, $x.a.length return $x'),
			[{order: 2, a: []}, {order: 1, a: []}, {order: 1, a: [1, 2]}], 'double order by object flowr');
			
		check(jsiq.query('for $x in ({val: 2}, {val: 1}, {val: 1}) group by $value := $x.val return {value: $value}'),
			[{value: 1}, {value: 2}], 'group by');
		
		check(jsiq.query('for $x in ({val: 2}, {val: 1}, {val: 1}) group by $value := $x.val return {value: $value, count: count($x)}'),
			[{value: 1, count: 2}, {value: 2, count: 1}], 'group by using aggregated');
		
		check(jsiq.query('for $x in ({val: 2, name: "john"}, {val: 1, name: "jim"}, {val: 1, name: "joe"}) group by $value := $x.val return {value: $value, names: [$x.name]}'),
		...
		check(jsiq.query('collection("captains").series[#1#]'),
		...
		check(jsiq.query('for $x in collection("captains") return $x.name'),
			["James T. Kirk", "Jean-Luc Picard", "Benjamin Sisko", "Kathryn Janeway", "Jonathan Archer", "Samantha Carter"], 'simple for with collection');
		
		check(jsiq.query('for $x in collection("captains"), $y in $x.series[] return { "captain" : $x.name, "series" : $y }'),
			[
		...
		check(jsiq.query('for $x at $position in collection("captains") return { "captain" : $x.name, "id" : $position }'),
			[
		...
		check(jsiq.query('for $captain in collection("captains"), $movie in collection("movies")[ $$.captain eq $captain.name ] return { "captain" : $captain.name, "movie" : $movie.name }'),
			[
		...
		check(jsiq.query('for $captain in collection("captains"), $movie allowing empty in collection("movies")[ $$.captain eq $captain.name ] return { "captain" : $captain.name, "movie" : $movie.name }'),
			[
				{ "captain" : "James T. Kirk", "movie" : "The Motion Picture" },
				{ "captain" : "James T. Kirk", "movie" : "The Wrath of Kahn" },
				{ "captain" : "James T. Kirk", "movie" : "The Search for Spock" },
		...
		check(jsiq.query('for $captain in collection("captains"), $movie in collection("movies") where $movie.captain eq $captain.name return { "captain" : $captain.name, "movie" : $movie.name }'),
			[
				{ "captain" : "James T. Kirk", "movie" : "The Motion Picture" },
				{ "captain" : "James T. Kirk", "movie" : "The Wrath of Kahn" },
				{ "captain" : "James T. Kirk", "movie" : "The Search for Spock" },
		...
		check(jsiq.query('for $x in collection("captains") order by $x.series.length, $x.name return $x'),
			[
				//there's an error in the standard example, The doctor is ordered by a null value, which is by default smaller
				//than anything else, the example puts it at the end. This would require an "empty greatest" clause.
				{ "name" : "Samantha Carter", "series" : [ ], "century" : 21 },
		...
		check(jsiq.query('for $x in collection("captains") order by $x.name descending empty greatest return $x'),
			[
				{ "codename" : "Emergency Command Hologram", "surname" : "The Doctor", "series" : [ "Voyager" ], "century" : 24 },
				{ "name" : "Samantha Carter", "series" : [ ], "century" : 21 },
				{ "name" : "Kathryn Janeway", "series" : [ "The next generation", "Voyager" ], "century" : 24 },
		...
		check(jsiq.query('for $x in collection("captains") group by $century := $x.century return { "century" : $century  }'),
			[ { "century" : 21 }, { "century" : 22 }, { "century" : 23 }, { "century" : 24 } ], 'group by with collection');
		
		check(jsiq.query('for $x in collection("captains") group by $century := $x.century return { "century" : $century, "count" : count($x) }'),
			[
				{ "century" : 21, "count" : 1 }, 
				{ "century" : 22, "count" : 1 },
				{ "century" : 23, "count" : 1 },
		...
		check(jsiq.query('for $x in collection("captains") group by $century := $x.century return { "century" : $century, "captains" : [ $x.name ] }'),
			[
